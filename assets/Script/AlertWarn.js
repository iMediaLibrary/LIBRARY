...
*/
Alert.show = function (title,detailString, enterCallBack, animSpeed) {
    ...
    cc.loader.loadRes("Alert", cc.Prefab, function (error, prefab) {
        ...
        // 获取子节点
        Alert._title = cc.find("Title", alert).getComponent(cc.Label);
        Alert._detailLabel = cc.find("content", alert).getComponent(cc.Label);
        Alert._enterButton = cc.find("known/Know", alert);
